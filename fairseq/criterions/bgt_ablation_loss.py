...
@register_criterion('bgt_ablation_loss')
class BGTAblationLossCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        recon_loss = self.reconstruction_loss * recon_loss
        ...
        if 'fr_trans_logits' in net_output:
            en_trans_loss, nll_loss = self.compute_loss(model, [net_output['fr_trans_logits']], en_target,
                                                        reduce=reduce)
    ...
    def aggregate_logging_outputs(logging_outputs):
        ...
        return {
            ...
            'recon_loss': sum(
                log.get('recon_loss', 0) for log in logging_outputs) / sample_size if sample_size > 0 else 0.,
            'trans_loss': sum(
                log.get('trans_loss', 0) for log in logging_outputs) / sample_size if sample_size > 0 else 0.,
